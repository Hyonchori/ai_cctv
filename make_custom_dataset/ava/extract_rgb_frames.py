...
def extract_rgb_frames(input_root, output_root, save=True, vis=False):
    aleady_names = [x for x in os.listdir(output_root) if os.path.isdir(os.path.join(output_root, x))]
    vid_names = [x for x in sorted(os.listdir(input_root)) if os.path.isfile(os.path.join(input_root, x))
